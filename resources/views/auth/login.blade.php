...
<body>
    <div class="login-card text-center">
        ...
        <p class="text-white mb-4" style="font-family: 'Libre Baskerville', serif;">Please enter your Email and Password</p>

        <form method="POST" action="{{ route('login') }}">
            @csrf

            <div class="mb-4 position-relative">
                <input type="email" name="email" id="email" class="form-control" placeholder="Username or Email" required autofocus autocomplete="username" value="{{ old('email') }}">
                <i class="bi bi-person form-control-icon"></i>
    <div class="x-input-error">{{ $errors->first('email') }}</div> <!-- Menampilkan pesan error di bawah input -->
            ...
            <div class="mb-3 position-relative">
                ...
                <div class="x-input-error">{{ $errors->first('password') }}</div> <!-- Menampilkan pesan error di bawah input -->
            </div>
        ...
        <p class="text-white">Doesn't have an account yet? <a href="{{ route('register') }}" class="text-white fw-bold">Register Now!</a></p>
    </div>
