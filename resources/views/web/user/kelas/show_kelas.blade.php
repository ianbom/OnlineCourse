        ...
        <div class="max-w-6xl mx-auto">
                    ...
                    <div class="w-full md:w-1/3">
                        @if ($course->image)
                            <div class="rounded-lg overflow-hidden shadow-lg">
                                <img src="{{ asset('storage/' . $course->image) }}" alt="{{ $course->name_course }}"
                            ...
                            <div class="flex gap-4 mt-6">
                                ...
                                @else
                                    <form action="{{ route('kelas.selesaikan', $course->id_course) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="bg bg-blue-300 p-1 rounded-lg text-white">Selesaikan</button>
                                    </form>
                                @endif
            ...
            <div class="mt-8">
                        ...
                        <div class="flex items-center justify-between bg-white rounded-lg p-4 border">
                            <div class="flex items-center gap-4">
                                @if ($materi->is_free)
                                    ...
                                    <div
                                        class="w-10 h-10 flex items-center justify-center bg-orange-500 rounded-full text-white">
                ...
                <div class="mt-8 text-center">
                    <a href="{{ route('kelas.belajar', $course->materi->first()->id_materi) }}"
                        class="bg-orange-500 text-white px-8 py-3 rounded-lg inline-block hover:bg-orange-600">
                        BELAJAR SEKARANG
                    </a>
                </div>
            </div>
