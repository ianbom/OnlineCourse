    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <p class="text-[18px] text-[#484848] leading-relaxed font-regular" style="font-family: 'Inter', sans-serif;">
                        {{ $course->description }}
                    </p>

                    <!-- Stats Bar -->
                    <div class="flex gap-3 mt-12">
                        <div class="flex items-center gap-2 bg-[#FDE4D3] px-4 py-2 rounded-[16px] border border-[#F58A44]">
                            ...
                            <span class="text-[#F58A44] font-inter">{{ $course->materi->count() }} Bacaan</span>
                        </div>
                        <div class="flex items-center gap-2 bg-[#C5AB9A] px-4 py-2 rounded-[16px] border border-[#6F2E03]">
                            ...
                            <span class="text-[#6F2E03] font-inter">{{ $course->materi->whereNotNull('video')->count() }} Video</span>
                        </div>
                        <div class="flex items-center gap-2 bg-[#FFEED0] px-4 py-2 rounded-[16px] border border-[#957F2E]">
                        ...
                        @else
                            <form action="{{ route('kelas.selesaikan', $course->id_course) }}" method="POST">
                                @csrf
                                <button type="submit" class="flex items-center gap-2 bg-[#B8805B] px-4 py-2 rounded-[16px] border border-[#B8805B] text-white font-inter font-medium text-[16px]">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                    </svg>
                                    Selesaikan
                        ...
                        @if ($checkSimpan)
                            <form action="{{ route('delete.save', $course->id_course) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex items-center gap-2 bg-[#6F2E03] text-white px-4 py-2 rounded-[16px] border border-[#6F2E03] font-inter font-medium text-[16px]">
        ...
        <div class="mt-16">
            <div class="flex justify-between items-center mb-4">
                <h2 class="font-semibold mb-4" style="font-family: 'Libre Baskerville', serif; font-weight: bold; font-size: 28px; color: #1A1A1A;">Konten tersedia</h2>
                <!-- CTA Button -->
                <a href="{{ route('kelas.belajar', $course->materi->first()->id_materi) }}" class="bg-[#F58A44] px-8 py-3 rounded-lg inline-block hover:bg-[#6F2E03]" style="font-family: 'Inter', sans-serif; font-size: 16px; color: white; font-weight: semibold;">
                    BELAJAR SEKARANG
                </a>
            </div>
            <div class="space-y-4">
