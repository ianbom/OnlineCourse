                ...
                @forelse ($materi as $item)
                    <tr class="text-gray-700">
                        ...
                        <td class="px-4 py-3 text-sm">
                            <div>
                                <p class="font-semibold">{{ $item->content->name_content }}</p>
                                <p class="text-xs text-gray-600">
                                    {{ Str::limit($item->description, 50) }}
                                </p>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-sm">{{ $item->name_materi }}</td>
                        <td class="px-4 py-3 text-sm">
                            <div class="flex space-x-2">
                                ...
                                <!-- Show Button -->
                                <a href="{{ route('materi.show', $item->id_materi) }}"
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-blue-600 rounded-lg focus:outline-none focus:shadow-outline-blue"
                                    aria-label="Show">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('materi.destroy', $item->id_materi) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                                    <button
