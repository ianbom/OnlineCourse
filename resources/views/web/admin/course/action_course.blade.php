<div class="flex space-x-2">
    ...
    <a href="{{ route('course.show', $course->id_course) }}"
        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-blue-600 rounded-lg focus:outline-none focus:shadow-outline-blue"
        aria-label="Show">
        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
    ...
    <form action="{{ route('course.destroy', $course->id_course) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus course ini?');">
        @csrf
        @method('DELETE')
        <button
